...
public class Map implements ActionListener {
	 ...
	 */
	// in order height, width, block, walker, pillar
	public Map() {
		this(20, 20, 20, 10, 10);
	}
	...
	public void putNew(int typ, int x, int y) {
		if (isEmpty(x, y)) {
			if (typ == Piece.JMAN) {
				jMan = new JMan(x, y, (int) Math.round(Math.random() * 2), this);
				grid[x][y] = jMan;
			} else if (typ == Piece.BLOCK) {
				grid[x][y] = new Block(x, y, this);
			} else if (typ == Piece.PILLAR) {
				grid[x][y] = new Pillar((int) Math.round(Math.random() * 2), this);
			} else {
				grid[x][y] = new Walker(x, y, (int) Math.round(Math.random() * 2), this);
			}
		}
	}
