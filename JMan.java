import java.awt.*;

/**
 * An instance of this class is a J*Man.
 * There should at most one J*Man in a game board at a time
 */
public class JMan extends Piece {
	/**
	 * Constructor: a new J*Man at position (x, y) on Map m
	 * with color red if c = 0, green if c = 1, and yellow if c = 2.
	 */
	public JMan(int x, int y, int c, Map m) {
		super(Piece.JMAN, m);
		// Complete this
		if (c == 0) super.setColor(Color.RED);
		else if (c == 1) super.setColor(Color.GREEN);
		else if (c == 2) super.setColor(Color.YELLOW);
	}

	/**
	 * Constructor: a new J*Man at position (x, y) on Map m
	 * with color c. Precondition: c is one of
	 * Color.RED, Color.GREEN, and Color.YELLOW.
	 */
	public JMan(int x, int y, Color c, Map m) {
		super(Piece.JMAN, m);
		// Complete this;
		super.setColor(c);
	}

	/**
	 * J*Man should move based on what button is pushed.
	 * This method is not used.
	 */
	@Override
	public void act() {
		return;
	}

	/**
	 * Move J*Man one step based on the value of i:
	 * 0 = up, 1 = down, 2 = left, 3 = right.
	 */
	public void step(int i) {
		setActed(true);

		if (i == 0) {
			if (!getMap().isEmpty(getX(), getY() - 1)) {
				Piece p = getMap().pieceAt(getX(), getY() - 1);
				if (checkColor(p)) {
					getMap().setPieceAt(null, getX(), getY() - 1);
					getMap().move(getX(), getY(), getX(), getY() - 1);
				}
			} else {
				getMap().move(getX(), getY(), getX(), getY() - 1);
			}
		} else if (i == 1) {
			if (!getMap().isEmpty(getX(), getY() + 1)) {
				Piece p = getMap().pieceAt(getX(), getY() + 1);
				if (checkColor(p)) {
					getMap().setPieceAt(null, getX(), getY() + 1);
					getMap().move(getX(), getY(), getX(), getY() + 1);
				}
			} else {
				getMap().move(getX(), getY(), getX(), getY() + 1);
			}
		} else if (i == 2) {
			if (!getMap().isEmpty(getX() - 1, getY())) {
				Piece p = getMap().pieceAt(getX() - 1, getY());
				if (checkColor(p)) {
					getMap().setPieceAt(null, getX() - 1, getY());
					getMap().move(getX(), getY(), getX() - 1, getY());
				}
			} else {
				getMap().move(getX(), getY(), getX() - 1, getY());
			}
		} else if (i == 3) {
			if (!getMap().isEmpty(getX() + 1, getY())) {
				Piece p = getMap().pieceAt(getX() + 1, getY());
				if (checkColor(p)) {
					getMap().setPieceAt(null, getX() + 1, getY());
					getMap().move(getX(), getY(), getX() + 1, getY());
				}
			} else {
				getMap().move(getX(), getY(), getX() + 1, getY());
			}
		}
	}

	public boolean checkColor(Piece piece) {
		if (piece == null) return false;
		if (getColor().equals(Color.RED) && piece.getColor().equals(Color.YELLOW)){
			setColor(Color.YELLOW);
			return true;
		}
		else if (getColor().equals(Color.GREEN) && piece.getColor().equals(Color.RED)) {
			setColor(Color.RED);
			return true;
		}
		else if (getColor().equals(Color.YELLOW) && piece.getColor().equals(Color.GREEN)) {
			setColor(Color.GREEN);
			return true;
		}
		else return false;
	}

	/**
	 * = representation of this piece
	 */
	public String toString() {
		String color = "";
		return getColorWord() + " J*Man at (" + getX() + ", " + getY() + ")";
	}
}

